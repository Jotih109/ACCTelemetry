# 🏎️ ApexView ACC — Dashboard de Telemetria para Assetto Corsa Competizione (MoTeC i2 Style)

> ⚠️ **PROJETO DEDICADO PARA ACC** — Dashboard profissional de telemetria em tempo real para **Assetto Corsa Competizione (ACC)**, inspirado nos layouts de engenharia de dados do **MoTeC i2 Pro**. Acompanha suporte a eletrônica GT3 (TC, TC Cut, ABS, Engine Map), clima dinâmico, pneus DHE, multi-carro e tela separada de análise pós-sessão ([mapa.pyw](mapa.pyw)).

---

## 📖 Sobre o Projeto

**ApexView ACC** é uma ferramenta de telemetria e análise de desempenho em tempo real desenvolvida em **Python**, **PyQt5** e **PyQtGraph** para o **Assetto Corsa Competizione (ACC)**. A aplicação acessa diretamente a **memória compartilhada** do jogo (`acpmf_physics`, `acpmf_graphics`, `acpmf_static`), mapeando os layouts expandidos de 800B/1588B/820B nativos do simulador GT3/GT4 da Kunos Simulazioni a 60 Hz sem a necessidade de plugins externos ou configurações de rede.

> 💡 **Conexão Zero-Config:** Basta abrir o dashboard e entrar na pista no ACC. O sistema detecta automaticamente o jogo, conecta, sincroniza dados e reconecta se você reiniciar a sessão ou trocar de carro/pista.
...
Pode abrir o dashboard antes ou depois do jogo. Ele fica aguardando o ACC e conecta sozinho quando você entra na pista — **não há nada a configurar no jogo**.

Sem o jogo aberto? `python mock_game.py` roda o simulador nativo de ACC ou `python main.pyw --mock` roda com o mock interno.
Quer só analisar as voltas salvas? `python mapa.pyw`.

**Pré-requisitos:** Windows (a memória compartilhada é lida via Win32/`ctypes`), Python 3.8+, Assetto Corsa Competizione (Steam).
